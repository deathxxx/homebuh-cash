...
<head>
    <meta charset=utf-8>
    <title><?php echo $settings['site_name']; ?> &mdash; <?php echo $lng->get( 199 ); ?></title>
    ...
    <style type="text/css">
        * {
        ...
        ul {
            ...
            background: transparent url("<?php echo $settings['static'];?>/loading.gif") 50% 50% no-repeat;
        }
    ...
    <script language="javascript">
        ...
        function ajax(url, _cb, np, post) {
            ...
            url = url + "xcsrf=<?php echo $settings['csrf'];?>";
        ...
        function getLineTpl(o) {
          ...
          <span class="amount ' + class_name + '">' + price(o.amount) + '<?php echo $settings['sign'];?></span>\
          <h2 class="name">' + o.nom + '</h2>\
          <span class="org">' + o.oname + '</span>\
          <span class="del"><a title="<?php echo $lng->get( 37 );?>" href="#" onclick="return del(' + o.id + ');">' +
                '<img src="<?php echo $settings['static'];?>/delete.gif"></a></span>\
        </li>';
        } //getLineTpl
        ...
        function refresh_list() {
            ...
            ajax("ajax/list.php?from=" + id("date_from").value + "&to=" + id("date_to").value + "&short=1", function (data) {
                ...
                if (data.length == 0) {
                    id("list").innerHTML = '<div id="no_lines"><?php echo $lng->get( 201 );?><div>';
                } else {
        ...
        function del(id) {
            if (confirm("<?php echo $lng->get( 41 );?>")) {
        ...
        function getGmapLocation(_fnc) {
            ajax("https://www.googleapis.com/geolocation/v1/geolocate?key=<?php echo $settings['g_key'];?>", function (s) {
...
<?php if ( ! empty( $auth ) ) : ?>
    <h3><?php echo $auth['msg']; ?></h3>
<?php endif; ?>
<?php if ( $usr->rghts['read'] != 1 ) :
	require_once( "lib/settings.php" );
	$set = new CashSett( $db, $usr );
	?>
    <form id="login_form" method="post">
        <div><label for="db"><?php echo $lng->get( 7 ); ?>: </label>
            <select id="db" name="login_db_name_list_cb">
				<?php
				$dbs = $set->getDbs();
				foreach ( $dbs as $db ) : ?>
                    <option value="<?php echo $db['id']; ?>"<?php if ( $db['id'] == $_COOKIE['DB_ID'] ): ?> selected<?php endif ?>><?php echo $db['name']; ?></option>
				<?php endforeach; ?>
            </select></div>
        <div><label for="login"><?php echo $lng->get( 6 ); ?>: </label>
            <select id="login" name="login_usr_name_list_cb">
				<?php
				$usrs = $set->getUsrNames();
				foreach ( $usrs as $u ) : ?>
                    <option value="<?php echo $u['id']; ?>"<?php if ( $u['id'] == $_COOKIE['USR_ID'] ): ?> selected<?php endif ?>><?php echo $u['name']; ?></option>
				<?php endforeach; ?>
            </select></div>
        <div><label for="pwd"><?php echo $lng->get( 3 ); ?>: </label><input type="password" id="pwd" name="password">
        </div>
        <div><input type="submit" value="<?php echo $lng->get( 2 ); ?>"></div>
    </form>
	<?php else : ?>
    <div id="add">
    <input type="button" id="add_btn" value="<?php echo $lng->get( 49 ); ?> ↓" onclick="return add_frm();">
	<?php if ( ! empty( $add_ret['msg'] ) ) : ?>
        <h3><?php echo $lng->get( 164 ) . ': '. $add_ret['msg']; ?></h3>
	<?php endif; ?>
    <form method="post" id="add_frm" onsubmit="return add(this);">
        ...
        <div><label for="cash_item_date"><?php echo $lng->get( 23 ); ?></label><input type="date" name="cash_item_date"
                                                                                      id="cash_item_date"
                                                                                      value="<?php echo date( 'Y-m-d' ); ?>"><img
                    id="lgif" src="<?php echo $settings['static']; ?>/loading.gif"></div>
        <div style="position:relative;">
            <label for="cash_item_nmcl_cb"><?php echo $lng->get( 17 ); ?></label><input type="text"
                                                                                        name="cash_item_nmcl_cb"
                                                                                        id="cash_item_nmcl_cb"
                                                                                        onkeyup="return fillcheck();"
                                                                                        onchange="return nomChange(this);"
                                                                                        onclick="return reloadLocation();">
            <div id="nmcl_list"></div>
        </div>
        <div><label for="cash_item_prod_type_cb"><?php echo $lng->get( 19 ); ?></label><input type="text"
                                                                                              name="cash_item_prod_type_cb"
                                                                                              id="cash_item_prod_type_cb">
        </div>
        <div><label for="cash_item_org_cb"><?php echo $lng->get( 200 ); ?></label><input type="text"
                                                                                         name="cash_item_org_cb"
                                                                                         id="cash_item_org_cb"></div>
        <div>
            <label for="cash_item_price"><?php echo $lng->get( 20 ); ?></label><input type="number"
                                                                                      name="cash_item_price"
                                                                                      id="cash_item_price">
            <label for="cash_item_qnt"><?php echo $lng->get( 21 ); ?></label><input type="number" name="cash_item_qnt"
                                                                                    id="cash_item_qnt" value="1">
        </div>
        <input name="cash_item_save" id="cash_item_save" type="submit" value="<?php echo $lng->get( 58 ); ?>">
    </form>
</div>
<div id="filter">
    <input id="date_from" type="date" value="<?php echo date( 'Y-m-d', time() - 3600 * 24 ); ?>"
           onchange="return refresh_list();" required> &mdash;
    <input id="date_to" type="date" value="<?php echo date( 'Y-m-d' ); ?>" onchange="return refresh_list();" required>
</div>
<ul id="list"></ul>
<div id="itog">=<span id="itog_sum">0</span><?php echo $settings['sign']; ?></div>
<?php endif ?>
